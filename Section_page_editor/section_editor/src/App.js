...
const InlineElement = ({ type, content, value, onChange }) => {
  switch (type) {
    ...
    case INLINE_TYPES.INPUT:
      return <input style={{fieldSizing: 'content',backgroundColor:"lightblue",padding:"1px"}} type="text" placeholder={content} value={value} onChange={(e) => onChange(e.target.value)} />;
...
const App = () => {
  ...
  const [userInputs, setUserInputs] = useState(() => {
    ...
    if (saved) {
      ...
      return lastTemplate?.userInputs || {};
    }
    return {};
  });
  useEffect(() => {
  ...
  const saveTemplate = () => {
    const template = {
      ...
      name: templateName,
      sections: sections,
      userInputs: userInputs,  // Add this line
  ...
  const loadTemplate = (template) => {
    ...
    setUserInputs(template.userInputs || {}); // Add this line
  };
